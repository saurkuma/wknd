...
const convertXF = (main, document) => {
  ...
  main.querySelectorAll('.experiencefragment').forEach((xf) => {
   ...
   const socialFacebook =  xf.getElementsByTagName('a')[0];
   const socialTwitter =  xf.getElementsByTagName('a')[1];
   const socialInsta =  xf.getElementsByTagName('a')[2];
   const div = document.createElement('div');
   div.append(image);
   div.append(title);
   div.append(description);
   div.append(socialFacebook);
   div.append(socialTwitter);
   div.append(socialInsta);
   cell.push([div]);
   });
